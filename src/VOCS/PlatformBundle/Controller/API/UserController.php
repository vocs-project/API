    ...
    public function postUsersListsWordsAction(Request $request)
    {
        ...
        if($user->getLists()->contains($list)) {
            ...
            if ($form->isValid()) {
                ...
                $word = $repWord->findOneBy(array('content' => $wordTrad->getWord()->getContent(), 'language' => $wordTrad->getWord()->getLanguage()));
                if($word != null) {
                    $wordTrad->setWord($word);
                }
                $trad = $repWord->findOneBy(array('content' => $wordTrad->getTrad()->getContent(), 'language' => $wordTrad->getTrad()->getLanguage()));
                if($trad != null) {
